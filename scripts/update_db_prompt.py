# -*- coding: utf-8 -*-
# 更新 DB 的 TRADE_SYSTEM_PROMPT：插入『主线判定注入与确认层』SOP (GET list -> PUT)
import requests
BT=chr(96)
BASE='http://localhost:8000/api/v1'
name='TRADE_SYSTEM_PROMPT'
def get_content():
    r=requests.get(BASE+'/prompts', timeout=30); r.raise_for_status()
    return r.json().get('prompts',{}).get(name, None)
def put(content, label):
    r=requests.put(BASE+'/prompts', json={'name':name,'content':content,'label':label}, timeout=30)
    r.raise_for_status(); return r.json()
content=get_content()
if content is None:
    print('DB 无该prompt'); raise SystemExit(1)
print('当前长度', len(content))
if '主线判定注入与确认层' in content:
    print('已存在,跳过')
else:
    marker='## 交易决策 SOP'
    add = ('## 主线判定注入与确认层\n\n'
    ...
    if marker in content:
        content=content.replace(marker, add+marker, 1)
    else:
        content=content+add
    res=put(content, '交易模式')
    print('PUT version', res.get('version'), 'len', len(res.get('content','')), '含SOP?', '主线判定注入与确认层' in res.get('content',''))
