# -*- coding: utf-8 -*-
"""backtest_wolf_t_consistency_v5.py — 候选(高召回) + 纪律/择机代理(room>=3%) 一致率"""
import os, sys, json
    ...
    for j in range(bi+2,len(bs)):
        ...
        if j>=hi_idx+2:
            ...
            if dry and closes[j]<=hi*up and closes[j]>closes[bi]*1.005 and any(closes[k]>=hi*0.98 for k in range(hi_idx+1,j+1)):
                return True
    return False
ELIGIBLE=['159516','588170','301018']; TARGET=3.0
...
print('候选 v2 + 纪律择机(room>=%.1f%%, wave只做T, 计划库armed, 首信号)' % TARGET)
print('%-10s %-12s | %-8s | %-8s' % ('date','wolf','buy','sell'))
rows=[]
for d in DAYS:
    bh=[]; sh=[]
    for c in ELIGIBLE:
    ...
    print('%-10s %-12s | %-8s | %-8s' % (d,WOLF[d][:12],(';'.join(bh) if bh else '-'),(';'.join(sh) if sh else '-')))
