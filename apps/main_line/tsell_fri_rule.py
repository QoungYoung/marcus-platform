...
def t_sell_v1(bars, look=8):
    ...
    return None
# 收集周五交易日, 分 confirm / no_confirm
fri_base=[]; fri_rule=[]  # base=confirm or day_end; rule=confirm or 提前14:30
for code in SYMS:
    ...
    for di,day in enumerate(days):
        ...
        if v1:
            fp=pnl(v1["price"])
            if is_fri(day): fri_base.append(fp); fri_rule.append(fp)
        else:
            dp=pnl(close)
            if is_fri(day):
                # 提前离场@14:30 = 买入后最后 <=14:30 的 bar close
                ex=None
                for b in reversed(bars[bi+1:]):
                    if hm(b)<=1430: ex=float(b["close"]); break
                ex=ex if ex is not None else close
                fri_base.append(dp); fri_rule.append(pnl(ex))
def stat(v):
    p=np.array(v); return "n=%d avg=%+.2f%% win=%d%% 盈=%d 亏=%d(%.0f%%) 亏均=%+.2f%%" % (len(v), np.mean(p), np.mean(p>0)*100, int(np.sum(p>0)), int(np.sum(p<0)), np.sum(p<0)/len(p)*100, np.mean(p[p<0]) if np.any(p<0) else 0)
print("== 周五确认制T出+未确认 整体 ==")
print("基线(确认/日终):", stat(fri_base))
print("周五未确认→提前离场@14:30(其余不变):", stat(fri_rule))
print()
print("== 仅看周五的 未确认 日(提前离场作用对象) ==")
nc_base=[fri_base[i] for i in range(len(fri_base)) if abs(fri_base[i]-fri_rule[i])>1e-9]
print("未确认日 基线(day_end):", stat(nc_base) if nc_base else "n=0")
