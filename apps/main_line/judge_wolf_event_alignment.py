...
规则：
  1) 只有具备 5min 动作回放的事件才进入一致率分子（v2: E05/E06/E08/E12/E13 等 8 事件；
     v3 追加 E01-E04: E01/E02=intent_open(开盘建仓/probe≤3%同日), E03/E04=stepwise B（253大盘急杀时点用真上证 index_5min_dh，E03/E04 窗口 2025-10-09 起已由 datahubco 补齐）；
...
def main():
    ...
    # 个股级主线内切换通道（E09-E12）：链级卖旧(出货周期) + 个股选买 → switch_executed 视为对齐
    sw = load("switch_stock_backtest.json") or {}
    sw_events = sw.get("events") or ([sw] if sw.get("event") else [])
    for sw_ev in sw_events:
        if not sw_ev.get("switch_executed"):
            continue
        for e in meas:
            if e["event"] == sw_ev.get("event"):
    ...
    if VER == "v3":
        caveats += ["v3: E01/E02 intent_open(开盘建仓/probe≤3%)为通道放行假设, 非盘口触发",
                    "v3: E03/E04 253时点=真上证5min(datahubco, 2025-10-09~2025-11-12 25交易日); 510300代理仅兜底缺失日",
                    "v3: E04 动作落在10-30(真上证253), Wolf原文为10-29低点买ETF, 差1交易日",
