...
方向体检(盘前/复盘一次)：
- sell_old: 持仓方向掉出 fusion TOP3 且 confirm stage∈{下跌中,证伪} → 建议卖腿(黄线/破位确认), 释放资金
- keep   : 方向掉出但 stage∈{缩量止跌,结构到位,突破候选,确认}(仍强/筑底) → "强的留"
- buy_new: fusion TOP1∪TOP2 内 stock_confirm 突破候选/确认活跃股、非拥挤 → 建议 254/253 低吸建新仓(试仓档额度)
默认 DRY: 只写 data/switch_builder_plan.json 与打印; SWITCH_AUTO_EXEC=1 才建议执行(接通另改)
"""
import json, os, sys
...
def active_stocks_by(stages):
    """fusion TOP1∪TOP2 概念内 stage∈stages 的活跃股 {code6: (theme, stage)}"""
    ...
    for cname, v in sc.items():
        if not isinstance(v, dict) or v.get("theme") not in top12: continue
        for s in (v.get("stocks") or []):
...
def build_plan():
    top3 = fusion_top3(); top12 = fusion_top3()[:2]
    tiers = {}
    sell_old, keep = [], []
    for p in held_positions():
        ...
        if dropped:
            if stage in WEAK_STAGES:
                ...
                keep.append({"symbol": sym, "stage": stage or "unknown(方向掉出,留观察)", "themes": ths})
        else:
    ...
    plan = {"top3": top3, "top12": top12, "sell_old": sell_old, "keep": keep,
            "buy_new": [{"code": k, "theme": v[0], "stage": v[1]} for k, v in buy_new.items()],
            "tiers": tiers, "escalate": {"ok": esc, "triggers": trig, "upgraded": upgraded},
            "auto_exec": os.getenv("SWITCH_AUTO_EXEC", "0") == "1", "dry": True}
    try:
