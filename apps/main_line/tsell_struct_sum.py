...
def struct_risk(bars):
    ...
    dm=max(closes)
    return bool(c<ma5 or (prev_vol>0 and day_vol<prev_vol*0.8) or (c>=dm*0.995 and prev_vol>0 and day_vol<prev_vol*0.9))
def run(use_struct):
    allt=[]; risk_skip=0
    for code in SYMS:
        ...
        for di,day in enumerate(days):
            ...
            if use_struct and struct_risk(bars): risk_skip+=1; continue
            allt.append(pnl(v1["price"]) if v1 else pnl(close))
    return allt, risk_skip
def rep(p, label):
    if not p: print("  %s: n=0" % label); return
    p=np.array(p); print("  %s: n=%d avg=%+.2f%% win=%d%% 亏=%d(%.0f%%) 总收益=%+.1f%%" % (label, len(p), np.mean(p), np.mean(p>0)*100, int(np.sum(p<0)), np.sum(p<0)/len(p)*100, np.sum(p)))
base,_ = run(False)
struct, rs = run(True)
print("== 总收益率对比 ==")
rep(base, "基线(所有做T)")
rep(struct, "结构降险(跳过结构风险日)")
print("  跳过数: %d" % rs)
print("  总收益变化: %+.1f%% (结构-基线)" % (np.sum(struct)-np.sum(base)))
