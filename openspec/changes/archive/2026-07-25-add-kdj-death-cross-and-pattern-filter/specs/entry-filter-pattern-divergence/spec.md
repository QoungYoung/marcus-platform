...
- **THEN** real body is 0.37, upper shadow is 2.50 (6.76x body), lower shadow is 0.58
...
- **THEN** the shooting star check SHALL use the most recent COMPLETED daily bar from `get_daily_kline_qfq` (not today's incomplete bar)
...
2. If pattern detection triggers hard block → final severity = 3 (Hard Block) regardless of overbought severity
3. If divergence detection triggers warning → final downgrade_multiplier = min(existing_multiplier, 0.5)
4. Pattern and divergence findings SHALL be appended to `LayerResult.details`

#### Scenario: Normal overbought but shooting star blocks
- **WHEN** RSI6=72, KDJ-J=80, CCI=123 (all normal severity 0)
- **AND** shooting star pattern detected (hard block)
...
- **AND** volume-price divergence detected (warning)
