...
系统 SHALL 每日先汇总所有 in-pit 行业（含指数级标的）当日计划定投金额；若计划总和超过可用现金（账户净值 × (1 - `cash_min_pct`)），按（tier 优先级，priority，权重）从高到低分配，直到现金耗尽，未分到的当日跳过、额度保留次日。

#### Scenario: 并发坑位超现金
- **WHEN** 当日 3 个行业计划金额合计 6 万，可用现金 3 万
- **THEN** 系统 SHALL 按优先级从高到低分配共 3 万（高优先级行业全额或按比例优先），低优先级行业当日跳过
