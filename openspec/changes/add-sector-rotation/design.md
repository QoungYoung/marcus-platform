...
## 验证结论（2026-08-16，08 与 05 双窗口三档对照）

- **机制验证全部通过**：行业预取/映射（index_member_all 5895 只）、强度因子、强势过滤（sector_excluded 事件）、轮动换仓（sector_switch：质量转弱卖出/换入/冷却）均可复现；08 窗口过滤剔除电子（5日-3.08%）等，05 窗口剔除 000066（-10.68%，过滤有效避险）
- **08 窗口（轮动市，08-06~08-14）**：基线 -0.15%（1 只 000506）vs 仅过滤 -0.15% vs 过滤+轮动 -0.13%（6 只、7 次换仓、realized_pnl 3901 翻倍）——轮动带来交易机会增量但总收益接近持平（换仓成本与卖出点抵消）
- **05 窗口（震荡市，05-18~05-29）**：基线 +0.77% > 仅过滤 +0.65% > 过滤+轮动 -0.91%——过滤误杀超跌反弹牛股 000767（+20.51%），轮动卖飞大涨票（质量转弱判定对涨高票误伤）
- **结论**：① 高切低只在行业分化大的轮动市有效（08 验证），震荡市（05）明显损伤 → rotation_enabled 默认关正确，建议按行业分化度条件启用；② 行业过滤降低风险（剔除弱势行业）但会误杀超跌反弹 → 生产默认不开，作为回测分析工具；③ 质量转弱卖出需要「趋势过热 vs 启动期」区分，避免卖飞（后续迭代）
