...
Defines the Layer 3 overbought filtering rules for the entry check system (`check_entry_filters`). Evaluates RSI6, KDJ-J, and CCI indicators with tightened thresholds and multi-indicator resonance detection to prevent entries at overbought extremes.
...
|-----------|-------|--------|
| < 150 | Normal | Pass |
| 150 - 200 | Warning | Probe position only, multiplier ≤ 0.5 |
...
- **WHEN** CCI is 175
- **THEN** CCI is graded "Only Probe" and triggers downgrade_multiplier ≤ 0.5
...
|------------|-------|--------|
| < 75 | Normal | Pass |
| 75 - 85 | Warning | Probe position only, multiplier ≤ 0.5 |
...
- **WHEN** RSI6 is 80
- **THEN** RSI6 is graded "Only Probe" and triggers downgrade_multiplier ≤ 0.5
...
|-------------|-------|--------|
| < 95 | Normal | Pass |
| 95 - 105 | Warning | Probe position only, multiplier ≤ 0.5 |
...
- **WHEN** KDJ-J is 100
- **THEN** KDJ-J is graded "Only Probe" and triggers downgrade_multiplier ≤ 0.5
...
The system SHALL compute a per-indicator severity level (0=Normal, 1=Probe, 2=Blocked, 3=HardBlock) for RSI6, KDJ-J, CCI, K-line patterns, and volume-price divergence, then apply resonance downgrade when two or more indicators are at severity ≥ 1.
...
4. Find the highest severity across all indicators (up to 5)
...
7. The shifted severity maps to the final grade:
   - 0 → Pass
   - 1 → Probe only
   - 2 → Blocked
   - 3 → Hard Block
...
- **AND** final severity stays 1 (Only Probe)
...
#### Scenario: Zijin Mining case would be caught
- **WHEN** RSI6 is 83.9 (severity 1) and KDJ-J is 102.8 (severity 1) and CCI is 250.2 (severity 2)
