...
// 用 AsyncLocalStorage 替代全局变量，防止并发请求互相覆盖
const _ctxStore = new AsyncLocalStorage<BacktestContext | null>();

/** 设置回测上下文（请求级隔离，并发安全） */
export function setBacktestContext(ctx: BacktestContext | null) {
  _ctxStore.enterWith(ctx);
  if (ctx) {
...
export function getBacktestContext(): BacktestContext | null {
  return _ctxStore.getStore() ?? null;
}
...
export const getQuoteTool = {
  ...
  async execute(_toolCallId: string, params: { symbol: string }, _signal?: AbortSignal) {
    ...
    if (isBacktest()) {
      ...
      const sign = chg >= 0 ? '+' : '';
      const ip = data.intraday_percentile;
      const ipStr = ip != null ? `  日内分位: ${ip.toFixed(0)}%${ip > 90 ? ' (追高风险)' : ''}` : '';
      const lines = [
        `${params.symbol} (回测 ${data.trade_date}${data.actual_time ? ' ' + data.actual_time.slice(-8) : ''})`,
        `现价: ${data.close}  涨跌: ${sign}${chg.toFixed(2)} (${sign}${chgPct.toFixed(2)}%)${ipStr}`,
        `开盘: ${data.open}  日内最高: ${data.high}  日内最低: ${data.low}`,
    ...
    const ip = q.intraday_percentile;
    const ipStr = ip != null ? `  日内分位: ${ip.toFixed(0)}%${ip > 90 ? ' (追高风险)' : ''}` : '';
    const lines = [
      `${q.name} (${q.symbol})`,
      `当前价: ${q.current}  涨跌: ${sign}${q.change} (${sign}${q.percent}%)${ipStr}`,
...
export const getConceptFundFlowTool = {
  ...
  label: '概念板块行情',
  description: '获取概念板块行情排行（按涨幅或主力资金流向排序）。数据源：东财push2实时(主力/超大单/大单/中单/小单净流入+板块广度+领涨股)，Tushare降级兜底。回测模式下每次交易窗口应重新调用获取最新值。sort_by=pct_change看涨幅榜，sort_by=main_net看资金榜',
  parameters: Type.Object({
  ...
  async execute(_toolCallId: string, params: { limit?: number; sort_by?: string }, _signal?: AbortSignal) {
    ...
    if (isBacktest()) {
      ...
      sectors.forEach((s: any, idx: number) => {
        ...
        const na = Math.abs(s.net_amount || 0);
        const net = na > 0
          ? `主力:${na >= 0.01 ? s.net_amount.toFixed(2) + '亿' : (s.net_amount * 10000).toFixed(0) + '万'}`
          : '';
...
export const getIndustryFundFlowTool = {
  ...
  label: '行业板块行情',
  description: '获取行业板块行情排行（按涨幅或主力资金流向排序）。数据源：东财push2实时(主力/超大单/大单/中单/小单净流入+板块广度+领涨股)。回测模式下每次交易窗口应重新调用获取最新值。sort_by=pct_change看涨幅榜，sort_by=main_net看资金榜',
  parameters: Type.Object({
    limit: Type.Optional(Type.Number({ description: '返回数量，默认15' })),
    sort_by: Type.Optional(Type.String({ description: '排序字段: pct_change(涨幅排行) / main_net(主力净流入排行)' })),
  }),
...
export const getMarketMoneyflowTool = {
	...
	label: '大盘资金流向',
	description: '获取沪深两市大盘资金流向（主力/超大单/大单/中单/小单净流入+买/卖分明细+总成交额）。数据源：东财push2实时(优先)+Tushare日频(降级)。回测模式下每次交易窗口应重新调用获取最新值。用于判断大盘整体资金情绪和主力动向',
	parameters: Type.Object({}),
	async execute(_toolCallId: string, _params: unknown, _signal?: AbortSignal) {
		...
		if (isBacktest()) {
			...
			const fmtBig = (v: number) => Math.abs(v) >= 0.01 ? v.toFixed(2) + '亿' : (v * 10000).toFixed(0) + '万';
			const lines = [
				`大盘资金流向 (回测 ${data.trade_date})`,
				`主力净流入: ${fmtBig(m.net_amount || 0)}`,
				`超大单: ${fmtBig(m.buy_elg || 0)}`,
				`大单: ${fmtBig(m.buy_lg || 0)}`,
...
export const getFinaMainbzTool = {
  name: 'get_fina_mainbz',
  label: '主营业务构成',
  description: '获取个股主营业务构成（产品/行业/地区维度的收入与利润占比）。数据源：Tushare fina_mainbz。用于分析公司核心收入来源、产业链定位。返回：各项业务的营业收入/营业利润/营业成本，按报告期排列',
  parameters: Type.Object({
  ...
  async execute(_toolCallId: string, params: { symbol: string; period?: string; limit?: number }, _signal?: AbortSignal) {
    const query = new URLSearchParams();
    if (params.period) query.set('period', params.period);
    if (params.limit) query.set('limit', String(params.limit));
    const qs = query.toString();
    const data = await apiFetch(`/indicator/fina-mainbz/${params.symbol}${qs ? '?' + qs : ''}`);
    if (data.error) throw new Error(data.error);
...
export const getExpressTool = {
  name: 'get_express',
  label: '业绩快报',
  description: '获取个股业绩快报数据（营业收入、利润、每股收益、净资产收益率及同比增长率）。数据源：Tushare express。用于快速了解公司经营业绩变化趋势，判断基本面强弱。返回：营收/营业利润/净利润/每股收益/ROE/同比增长',
  parameters: Type.Object({
    symbol: Type.String({ description: '股票代码，如 SH600519、SZ000001 或纯数字 600519' }),
  ...
  async execute(_toolCallId: string, params: { symbol: string; period?: string; limit?: number }, _signal?: AbortSignal) {
    ...
    if (params.limit) query.set('limit', String(params.limit));
    const qs = query.toString();
    const data = await apiFetch(`/indicator/express/${params.symbol}${qs ? '?' + qs : ''}`);
    if (data.error) throw new Error(data.error);
...
export const checkEntryFiltersTool = {
  ...
  async execute(_toolCallId: string, params: { symbol: string; sector_net_inflow?: number; volume_ratio?: number }, _signal?: AbortSignal) {
    ...
    if (isBacktest()) {
      ...
      const ip = data.intraday_percentile ?? t.intraday_percentile;
      const ipStr = ip != null ? ` | 日内分位: ${ip.toFixed(0)}%${ip > 90 ? ' 🚫追高风险' : ip > 70 ? ' ⚠️偏高' : ''}` : '';
      lines.push(`当前价: ${data.current_price} | MA5: ${data.ma5} | MA20: ${data.ma20} | RSI6: ${data.rsi_6}${ipStr}`);
      lines.push('');
      ...
      const net5d = l2.main_net_5d || 0;
      const absNet = Math.abs(net5d);
      let netStr: string;
      if (absNet < 0.005) {
        netStr = '≈0 (资金中性)';
      } else if (absNet < 1) {
        netStr = `${(net5d * 10000).toFixed(0)}万`;
      } else {
        netStr = `${net5d.toFixed(2)}亿`;
      }
...
export const listLTCandidatesTool = {
  ...
  async execute(_toolCallId: string, params: { status?: string }, _signal?: AbortSignal) {
    const query = new URLSearchParams();
    if (params.status) query.set('status', params.status);
    const qs = query.toString();
    const data = await apiFetch(`/lt-pool/candidates${qs ? '?' + qs : ''}`);
    if (data.error) throw new Error(data.error);
    const lines: string[] = [];
...
export const addLTCandidateTool = {
  ...
  async execute(_toolCallId: string, params: { symbol: string; name?: string; chain_name?: string; chain_role?: string; notes?: string }, _signal?: AbortSignal) {
    const data = await apiFetch('/lt-pool/candidates', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export const removeLTCandidateTool = {
  ...
  async execute(_toolCallId: string, params: { symbol: string }, _signal?: AbortSignal) {
    const data = await apiFetch(`/lt-pool/candidates/${encodeURIComponent(params.symbol)}`, {
      method: 'DELETE',
    });
    if (data.error || data.detail) throw new Error(data.detail || data.error);
...
export const updateLTCandidateTool = {
  ...
  async execute(_toolCallId: string, params: { symbol: string; notes?: string; chain_name?: string; chain_role?: string }, _signal?: AbortSignal) {
    const data = await apiFetch(`/lt-pool/candidates/${encodeURIComponent(params.symbol)}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(params),
    ...
    if (data.error || data.detail) throw new Error(data.detail || data.error);
    return { content: [{ type: 'text', text: `✅ ${data.symbol} 已更新` }], details: data };
  },
};
...
export const getIntradayMinTool = {
  ...
  async execute(_toolCallId: string, params: { symbols: string; freq?: string }, _signal?: AbortSignal) {
    const freq = params.freq || '1min';
    const data = await apiFetch(`/market/intraday-min?symbols=${encodeURIComponent(params.symbols)}&freq=${freq}`);
    if (data.error) return { content: [{ type: 'text', text: data.error }], details: data };
...
export const CHAT_TOOLS = [
  ...
  readDbTableTool,
  getDbSchemaTool,
  listLTCandidatesTool,
  addLTCandidateTool,
  removeLTCandidateTool,
