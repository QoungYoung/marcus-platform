...
⚠️ **止盈/止损前必须执行的检查（每轮扫描，下单前强制执行）**：
  1. 调用 get_orders() 查看是否有同股票的活跃卖出委托 → 有则跳过，等成交或撤单
  2. 调用 get_portfolio() 获取该股票的实际剩余持仓数量，用于计算卖出股数
  3. 检查自己的交易报告历史：本轮交易中是否已对该股票执行过同一档位的止盈
  4. 若同一档位已执行过 → 跳过，等待触发下一档位（15%或20%）再执行
  5. 卖出数量 = 剩余持仓 × 1/3（100股取整），不是原始买入数量 × 1/3
...
- 分批止盈（趋势完好则持有，趋势走弱才止盈 — 右侧交易核心）：
  ⚠️ **止盈前置条件（每次止盈前必须检查，缺一不可）**：
    调用 get_realtime_indicators(symbol) 和 get_moneyflow(symbol) 判断趋势：
    - ✅ 趋势完好（价>MA5 AND MA5>MA20 AND 主力净流入>0 AND 板块涨幅>0）
      → **推迟止盈**，让利润奔跑（右侧交易核心原则）
      → 仅记录但不执行，下一轮扫描重新判断
    - ⚠️ 趋势走弱（价<MA5 OR 主力净流出 OR 板块转跌）
      → **执行止盈**，锁定利润
    - 🚫 极端盈利 ≥ 20% → 无论趋势如何，无条件执行清仓（铁律）
  ⚠️ **止盈去重机制**：
    ...
    - 判断方法：下单前先调用 get_orders 检查是否有同股票同方向的活跃订单
      ...
      若已对这只股票执行了 ≥10% 止盈 → 本次跳过 10% 档，等待 15% 再触发
  **止盈执行规则**：
    - 盈利 10-15% → 趋势走弱时卖 1/3，趋势完好则持有
    - 盈利 15-20% → 趋势走弱时再卖 1/3，趋势完好则持有
    - 盈利 ≥ 20% → 无条件清仓（不看趋势，锁定暴利）
