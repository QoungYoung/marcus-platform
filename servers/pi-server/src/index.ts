...
**第三步：选股分析**
  ⚠️ 行业聚焦原则（防止频繁切换赛道、无法形成复利）：
    - 持仓已涉及某产业链 → 优先在该产业链内加仓/换仓，而非跳去新概念
    - 同时持有的概念/产业链 ≤ 2 个（聚焦才能形成深度认知和复利）
    - 切换产业链需满足：原产业链板块涨幅 < 1% 连续 3 个窗口 或 资金持续流出
    - 禁止行为：每天买入不同行业的新标的 → 5笔交易横跨5个行业 = 系统失败
  *产业链组合构建流程（优先于单票分析）：**
  a. **锁定主线** — 从 hot_concepts 或 get_concept_fund_flow 前5名中选1-2条主线
  ...
  *单票技术面检查（对组合中每只票执行）：**
    ...
    ⚠️ 买入回踩确认规则（禁止追在最高点）：
      - 标的当日涨幅已超过 3% → 禁止立即买入，必须等价格回踩 MA5 或当日分时均线
      - 回踩不破 MA5/分时均线 → 可在回踩位买入（这才是右侧买点）
      - 回踩跌破 MA5 → 趋势可能转弱，放弃本次建仓
      - 例外：板块龙头 + 涨停不适用（涨停无法回踩），此时按涨停次龙头规则处理
    ...
    - 排除缩量上涨股（量比 < 0.8 且价格上涨）
      例外：趋势确认日（扫描报告 price_action_confirm=True 或市场立场 green），量比门槛放宽至 0.5
...
⚠️ 铁律二：盈利单不能变亏损（移动止盈保护）：
  1. 持仓浮盈 ≥ 3% → 止损线自动上移至「成本价 + 1%」（永不亏损出局）
  2. 浮盈 ≥ 5% → 止损线上移至「成本价 + 2%」
  3. 浮盈 ≥ 8% → 止损线上移至「成本价 + 4%」，同时执行分批止盈规则
  4. 目的：消灭「浮盈+4.74%→最终亏损-2.44%」的致命模式（东睦股份典型案例）

⚠️ 铁律三：大盘大涨日禁止机械止损（锁仓模式）：
  1. 当日任一主板指数（上证/深证/创业板/科创50）涨幅 > 1% → 自动锁仓
  2. 锁仓期间：禁止执行任何止损卖单（包括 2% 止损、趋势破位止损）
  3. 例外：个股突发利空（跌停封板/停牌/重大负面公告）→ 可执行止损
  4. 锁仓的止损延迟到下一个非大涨日执行
  5. ⚠️ 锁仓解除条件（防止高开低走误锁）：
     a. 触发锁仓的指数从日内最高点回落 > 0.8%
       且 当前指数点位 < 开盘价 → 确认高开低走 → 立即解除锁仓
     b. 解锁后恢复正常止损规则，按当前位置重新评估所有持仓
     c. 例：大盘高开+1.5%触发锁仓 → 10:30回落至+0.3%且低于开盘价 → 解锁 → 正常止损
  6. 目的：防止「大盘+1.37%时割肉」，同时防止「高开低走被锁仓陪跌」

- 止损：浮动亏损 ≥ 2% 且持仓非今日买入 → 卖出（先检查铁律三锁仓状态）
- 分批止盈：盈利 ≥ 10% 卖 1/3，≥ 15% 再卖 1/3，≥ 20% 清仓（仅限非今日买入）
- 趋势破位（收盘跌破 MA5 或 MACD 死叉）且持仓非今日买入 → 减仓 50% 或全平（先检查铁律三）

⚠️ 止损后强制补位（防止止损后空仓踏空）：
  1. 止损卖出后，立即从该股所在板块涨幅榜重新扫描候选
  2. 若原板块仍有标的满足条件（涨幅前 5 + 右侧信号 + 未涨停）→ 优先补仓同板块次龙头
  3. 若原板块整体走弱（板块涨幅 < 1% 或资金流出）→ 跨板块搜索新主线
  4. 补位仓位 = 止损标的原仓位 × 0.8（略保守，错过第一波需更谨慎）
  5. 补位标的不受 T+1 锁定（是买入操作）
