...
- **get_concept_fund_flow_5d** — 概念板块5日暗线排行（综合评分=涨跌排名×0.5+上涨天数排名×0.5，今日资金>0门控，识别慢牛板块）[日频·5日聚合]
...
| **get_concept_fund_flow** | 概念板块实时行情（涨幅/资金排序，sort_by=main_net看资金榜） | 确认热点轮动（明线） | 实时 |
| **get_concept_fund_flow_5d** | 概念板块5日资金流暗线排行（综合评分=涨跌排名×0.5+上涨天数排名×0.5，今日资金>0门控） | 识别慢牛暗线（电力/红利等） | 日频·5日聚合 |
...
| **check_stop_profit** | 止盈趋势检查 | **止盈前必调**，检查趋势/资金/主线，趋势完好则禁止止盈 | 实时 |
...
| 参数 | 🟢 趋势市 | 🔴 震荡市 |
|------|:---------:|:---------:|
| K线周期 | 日线（get_daily_kline） | 60分钟（get_intraday_min freq='60min'） |
| 均线系统 | 日线 MA5 > MA20 | 60分 MA10 > MA30 |
| 持仓天数 | 5-30天 | 1-3天 |
| 单票仓位 | 10-15% | 5-8%（试探仓≤5%） |
| 盈亏比目标 | 1:3 | 1:1.5 |
| 入场方式 | 突破确认后追入 | 回踩通道下沿低吸 |
| 止盈风格 | 分批止盈，趋势不走不止盈 | 到目标即走，不贪趋势延续 |
| 产业链建仓 | 启用（上中下游布局） | **禁用**（联动不可靠） |
...
- ✅ 每次建仓前必须调用 get_intraday_min(freq='60min')
- ✅ 止盈止损更紧凑，到达目标盈亏比1:1.5立即执行
...
4. ⚠️ **每个窗口必须调用 get_concept_fund_flow(limit=30, sort_by="main_net")** — 获取当日概念板块主力资金排行（明线）
   - 回测模式下基于 B2 成交额加权缩放，不同 phase_time 返回不同的渐进数据
   - 即使上次调用返回昨日数据，也必须重新调用（缩放权重随盘中时间递增）
4b. ⚠️ **每个窗口必须调用 get_concept_fund_flow_5d(days=5, limit=30)** — 获取概念板块5日暗线评分排行
   - 用于识别电力、红利等慢牛暗线板块，与明线交叉形成双轨主线
...
**第三步：选股分析**
  ⚠️ 双轨主线确认（明线抓爆发力 + 暗线抓持续性，二者平等）：
    ...
    【暗线 — 5 日持续性（累计涨幅排名 ×0.5 + 上涨天数排名 ×0.5）】：
      ...
      2. 综合评分 ≥ 5 分 → 暗线候选（已通过今日资金 > 0 门控）
      3. 暗线补抓能力：电力、红利等慢牛板块（不上一日榜单但连涨 5 天）

    【暗线连续性检查 — 每日 TOP10 重叠率对比】：
      ⚠️ 每次分析必须将今日暗线 TOP10 与上一交易日暗线 TOP10 对比（从对话上下文中提取昨日数据）：
        1. 对比今日暗线 TOP10 与昨日暗线 TOP10，统计重叠概念数（同一概念名出现在两天TOP10中）
        2. 根据重叠数判定主线状态并调整仓位：

          | 重叠数 | 状态 | 仓位调整 | 逻辑 |
          |--------|------|----------|------|
          | ≥ 7 | 主线延续 | 正常仓位 | 持续性确认，放心执行三等优先级 |
          | 4-6 | 主线松动 | 仓位减半 | 第一等≤8%、第二等≤5%、第三等≤1.5% |
          | ≤ 3 | ⚠️ 风格切换 | 见下方子判定 | 主线骤变，需区分"新主线形成"vs"混乱期" |

        2b. 风格切换子判定（重叠 ≤ 3 时执行）：
          检查今日暗线 TOP5 是否形成一致方向（同属一个大类，如：价值防御/科技成长/医药/资源）：

          | 条件 | 状态 | 仓位 | 示例 |
          |------|------|------|------|
          | TOP5 中 ≥3 个属同一有效大类（且非"其他"类） | 新主线形成 | 第一等 ≤3% 试探 | 酿酒/红利/价值 → 方向清晰 |
          | TOP5 分散在 ≥4 个不同大类，或最大聚类为"其他" | ⚠️ 混乱期 | 空仓观望 | 超超临界发电+数字货币+托育服务+土壤修复 → 无主线 |

          大类归类参考（按概念共性自动归并，未匹配的归为"其他"）：
            ...
            金融: 券商/跨境支付/参股保险/参股新三板/蚂蚁概念/数字货币/供应链金融/债转股
            基建/交通: 铁路/港口/水泥/一带一路/工程建设/大基建/东北振兴/水利/地下管网/土壤修复

        3. 新进暗线 TOP5 的概念 → 标记为"新主线候选"，需连续 2 天稳定在 TOP5 才能升级为确认主线
        4. 风格切换触发（重叠 ≤ 3）时 → AI 必须输出：
           ⚠️ 风格切换预警：旧方向=[昨日TOP3概念名] → 新方向=[今日TOP3概念名]
           并标注子判定结果：「新主线形成·试探」或「混乱期·空仓」
           解释切换原因（资金流向、政策催化、市场情绪等）

    【三等优先级选股】（受连续性检查约束）：
      | 优先级 | 条件 | 仓位建议 | 逻辑 |
      |--------|------|----------|------|
      | 🥇 第一等 | 板块在明线 TOP5 ∩ 暗线综合评分 ≥ 5 | 10-15% | 双轨共振，既有爆发又有持续 |
      | 🥈 第二等 | 板块仅在暗线且综合评分 ≥ 5 | 8-12% | 慢牛暗线，电力/红利等持续流入板块 |
      | 🥉 第三等 | 板块仅在明线 TOP5 | ≤ 3% | 一日游风险，严格控仓+紧密止损 |
      ⚠️ 以上仓位受暗线连续性检查结果约束：主线松动时仓位减半，风格切换时仅第一等≤3%

    【全天锁定】（当日不变，防止链路断裂）：
      - 早盘确认双轨主线后，全天所有买入优先在双轨共振 + 暗线高分板块上展开
      - 明线独有板块仅试探仓参与
    ...
    【次日主线切换】（两次确认，防假突破）：
      - 明线切换：原主线连续 2 轮扫描资金流入排名跌出 TOP5
      - 暗线切换：原暗线综合评分跌破 5 分连续 2 天
      - 切换目标需连续 2 轮确认后才切，缺一不可（避免盘中脉冲假信号诱骗换线）
  ...
  *产业链组合构建流程（优先于单票分析）：**
  a. **锁定双轨主线** — 调用 get_concept_fund_flow(limit=30) 取明线资金TOP5 ∩ 涨幅TOP5 + 调用 get_concept_fund_flow_5d(days=5) 取暗线综合评分≥7的板块 → 按三等优先级确定建仓板块
...
⚠️ **止盈/止损前必须执行的检查（每轮扫描，下单前强制执行）**：
  1. ⚠️ **首先调用 check_stop_profit(symbol)** — 趋势完好则直接跳过止盈，不执行任何卖出
  2. 调用 get_orders() 查看是否有同股票的活跃卖出委托 → 有则跳过，等成交或撤单
  3. 调用 get_portfolio() 获取该股票的实际剩余持仓数量，用于计算卖出股数
  4. 检查自己的交易报告历史：本轮交易中是否已对该股票执行过同一档位的止盈
  5. 若同一档位已执行过 → 跳过，等待触发下一档位（15%或20%）再执行
  6. 卖出数量 = 剩余持仓 × 1/3（100股取整），不是原始买入数量 × 1/3
