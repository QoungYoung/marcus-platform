...
> 生成：2026-09-01；更新：2026-09-04（第三次切换上下文快照；commit 见 git log。本轮完成 Wolf 选股/做T/风控对齐，含六因子/board_half/轮动去弱留强/产业链形态提示词/不再注入震荡趋势市状态）
...
## 0. 下一位开发者待办（明确未做 · 2026-09-04 交接）

> 本轮已把 Wolf 选股/做T/风控对齐进生产（主线判定/成分股确认/高低位/confirm_chain/浪型门/做T含申万行业防御/周末降险/六因子wolf_judge软指导/check_entry_filters Wolf对齐/board_half减半/主线内轮动去弱留强+龙头+相对强度/产业链形态提示词(让agent自查)/不再注入震荡趋势市状态）。**下述两块仍未实现，作为下一开发重点：**

### A. 风控·查杠杆（P2）❌ 未做
- 现状：只有 macro_state 的宏观两融（余额/20日/净买）+ P2 Gate 的 macro 'margin_burst' 开关；'/indicator/safety-margin/{symbol}' 端点存在但**未接入候选/持仓风控链**。
- 要做：个股/持仓级杠杆·融资盘检查——两融余额、融资买入占比、杠杆资金集中度、融资盘占流通市值比；命中（高杠杆+资金流出/高换手）→ 降级或阻断新开。
- 落地：放入 check_entry_filters 或 p2_entry_gate 的 risk 维度；数据源 tushare margin_detail/margin（个股两融）；复用 check_safety_margin 从宏观落到个股级。

### B. 宏观机构行为 / 持仓纪律 / 复盘（P2/P3）❌ 未做
- **机构行为**：macro_state 仅宏观级（两融/GJD份额/北向/龙虎榜外资）；缺**板块级/个股级机构行为**——公募/险资/社保持股、基金重仓(与拥挤PIT联动)、板块级外资持续性。
- **持仓纪律**：backend 检索=0，未做；需加 仓位上限/总回撤保护线/连亏/持仓集中度/单票上限 等纪律检查（可落 position_tier_monitor 或独立任务）。
- **复盘**：每日/每周交易复盘——计划→命中、买卖对错、胜率/盈亏归因、候选→建仓→止盈止损链路复盘，产出复盘报告+行动项。现有 daily_review_enhanced / t_ai_agent.ai_daily_review 仅部分覆盖（做T/信号层），缺完整买卖/持仓纪律复盘闭环。
