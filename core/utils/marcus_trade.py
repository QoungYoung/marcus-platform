...
class MarcusVNPyExecutor:
    ...
    def get_account(self) -> dict:
        ...
        conn = sqlite3.connect(str(data_dir / "trades.db"))
        cursor = conn.cursor()
    ...
    def buy(self, symbol: str, price: float, volume: int, reason: str = "") -> dict:
        ...
        # 直接扣款并增加持仓 (简化流程)
        self.engine.available_cash -= total_cost
        # 使用 Position dataclass
        from paper_engine import Position
        ...
        # 创建订单记录
        order_id = f"ORD{self.engine.order_counter:06d}"
        ...
        # 保存成交记录到数据库 (直接插入 trades 表)
        import sqlite3
        conn = sqlite3.connect(self.engine.db_file)
        cursor = conn.cursor()
        cursor.execute('''
            INSERT INTO trades (orderid, symbol, direction, price, volume, amount, profit, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (order_id, symbol, '买入', price, volume, total_cost, 0, datetime.now().isoformat()))
        conn.commit()
        conn.close()
