...
**数据口径（全部来自他的原话或本仓既有模块，不自造）**
  ...
  · 指数下支撑 / 上方压力位 = **指数前低 / 近 20 日最高**（他的用法里"前低=支撑、前高=压力"，
    见 2026-05-13「冲击前高4230.11压力位」/ 2025-11-23「上面压力对应3922附近」）。
    ⚠️ 生产实测 `support_resistance.compute_levels('sh000001')` 对指数**取不到日线**（返回空）
    → 该分支实际不可用，真正生效的是上面这套；返回值里 `src` 永远标注来源，不静默。
...
def index_levels(force: bool = False) -> Dict[str, Any]:
    ...
    ⚠️ **实测（生产 2026-09-11）**：本仓既有 `support_resistance.compute_levels('sh000001')`
    对**指数**取不到日线（`get_daily_bars('000001.SH')` 返回 `[]`）→ 它对我们实际不可用，
    所以**实际生效的是回退口径**：前低（支撑）/ 近 20 日最高（压力）。
    仍保留先试 SR 的分支（若将来指数日线打通就自动切回去），并且**永远返回 src 标注来源**。
    """
    try:
