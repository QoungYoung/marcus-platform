...
def build_sizing(symbol: str, price: float, net_asset: Optional[float] = None,
    ...
    if mode in ("trend_break", "vrebounce", "vreb_etf", "mom_etf"):
        # 短线档共用（V反/科技ETF/动量趋势/趋势突破 同规模口径：单笔30%/单标30%/总60%）
...
def build_t_position(symbol: str, price: float, volume: Optional[int] = None,
    ...
    if build_mode in ("trend_break", "vrebounce", "vreb_etf", "mom_etf"):
        skip_timing = True  # 突破/V反/科技ETF/动量趋势 建仓不要求回踩企稳
