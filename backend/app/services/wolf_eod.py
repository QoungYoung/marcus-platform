...
处置：① 三者的 cron 挪到 EOD 之后（18:40 / 18:50 / 19:40）；② 任务内先做**就绪检查**，
   ...
   ⚠️ 「交给调度器重试」在 2026-09-16 之前是**空话**——`settings.retry` 只是死配置，
   `scheduler_service` 里没有任何 retry 实现（当天失败即终止）。现已实现：**只对显式
   opt-in 的 script 任务**（`retry: {enabled: true}`，见 config/tasks.yaml 的盘后 EOD 批）
   在失败后按 `delay_seconds` 自动重跑，最多 `max_attempts` 次。
