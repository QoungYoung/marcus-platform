...
async def check_entry_filters(req: EntryCheckRequest):
    ...
    # ── 拥挤无空间黑名单：已于 2026-09-13 真删 ──
    # 原 block(2026-09-03 上线)：读 data/crowding_blacklist.json → 公募核心拥挤个股
    #   （n_funds≥4 且 Σfloat≥1%）无个股空间时 hard_block=True（含 _crowd_space_reason 个股豁免）。
    # 删除依据(D15 事件研究 docs/wolf-d15-crowd-blacklist-event-study.md + 用户 2026-09-13 指令)：
    #   被拦组未来 5 日 +3.689% vs 非拥挤 +0.250%（块状 t=1.39 不显著）→ 拦反；
    #   「公募重仓」这条腿无统计支撑（D11：主题层 IC≈0、个股层 IC +0.019、加排除后基线 +0.683%→−0.152%）。
    # 生成侧（rotation_universe.build_crowding_blacklist）与消费侧（wolf_confirm_pick /
    #   rotation_switch_arm / rotation_switch_agent 的静默跳过）一并删除，勿重新引入。
