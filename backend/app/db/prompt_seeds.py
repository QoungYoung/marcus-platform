...
- **t_only（B反/调整）**：主基调=**只做T不新建仓**——底仓不动，T仓正T低吸(指数回撤2-3%)/分时T出/黄线离场；午后/尾盘更不新开
  - ⚡ **t_only 主线例外（2026-09-09, Wolf 对齐：大盘调整浪内照做已确认主线）**：若 main_line_state.mainline_gate 的 confirmed_candidate 非空——该主线(如当前农业)为"资金四重确认+结构曾确认(2浪回调不破前低再新高)"，**不受只做T限制**：允许在其回调低吸位/曾确认窗内按 P3 intent=new_base/probe 建底仓(单票≤5%、合计≤10%)；加仓仍需回调不破前低/放量突破确认，破位收盘即撤。watch/reserve 及未确认主题一律仍只做T。
