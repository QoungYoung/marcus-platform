...
1. `get_concept_fund_flow(limit=30)` 拉取资金和涨幅两个维度
2. 取 **资金 TOP10 ∩ 涨幅 TOP15** → 当日主线候选池
3. 排除"涨幅高但资金为负"的概念
4. 选 1-2 个最强概念为当日主线（优先同时入资金+涨幅 TOP10）
5. 检查 signal_level：⚡极端(>40亿+≥2涨停) / 🔥偏强(15-40亿+≥1涨停) / 📊常规
6. 非今日时间戳 → 不可用
...
g. **逐只技术面检查**（对计划表中每只标的执行）：
   ...
   **第三层 — 超买过滤**（RSI6/KDJ-J）
   - RSI6 ≥ 90 → 🚫禁止 | 85-90 → ⚠️仅试探仓 | < 85 → 🟢正常
   - KDJ-J ≥ 110 → 🚫禁止 | 105-110 → ⚠️仅试探仓 | < 105 → 🟢正常
   
   **返回结果**包含：降仓系数(×1.0/×0.5/×0.0)、最大仓位%、买入确认规则

h. **买入确认规则**（每只标的独立执行）：
   - 标的当日涨幅 < 3% → 直接入场
   - 3-5% → 等 3-5 分钟横盘不破均线
   - 5-8% → 等 2-3 分钟，量比 > 1.5 才入场
   - > 8% → 放弃（不追涨）
   - **等待期间可并行准备下一只候选标的的数据**（调取 quote/indicators/technical），但禁止在横盘确认完成前对同一只标的下单
   - 15 分钟内跌破均线 → 趋势不稳定，从计划表中移除此标的
   - **龙头涨停（涨幅 > 9.5%）→ 不追龙头**，可买同概念次龙头（需满足 5日主力>0 + RSI6<85 + J<105 + 换手 2-15%）
