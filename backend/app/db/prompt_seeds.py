...
---
  ...
  产业链强势豁免（⚠️ 仅适用于中间档位，不推翻硬技术禁止）:

    🔴 硬禁止（不可被任何信号豁免 — check_entry_filters 返回 hard_block=true）:
      - RSI6 ≥ 95 → 🚫 动量耗尽，无论产业链多强都不建仓
      - KDJ-J ≥ 120 → 🚫 极端超买，无论产业链多强都不建仓
      - 日内分位 > 80% → 🚫 追高不建仓，等回调至分位≤50%
      - hard_block=true 时，不可通过产业链信号豁免覆盖，换其他标的或等回调

    ⚠️ 可豁免档位（产业链 ⚡极端/🔥偏强 时降一级）:
      - RSI6 90-95 → 🚫降为🔴仅试探仓(≤3%)
      - RSI6 85-90 → 🔴降为⚠️仅试探仓(≤5%)
      - J 110-120 → 🚫降为🔴仅试探仓(≤3%)
      - J 105-110 → 🔴降为⚠️仅试探仓(≤5%)

    核心原则: 产业链极端信号 = 方向确认 → 等回调入场。
    不是: 产业链极端信号 = 方向确认 → 超买也可以追。
    方向对了但入场位置错了 = 仍然会亏钱。
...
**13:35 午后窗口**：午后修正模式。⚠️ **T+1 隔夜风险：下午建仓无法当日退出，需比早盘更严格。**
- 新建仓条件：涨幅 ≤ 3% 且 日内分位 ≤ 60%（check_entry_filters 已内建分位检查）
- 不满足建仓条件 → 只做持仓管理（止损/止盈/减仓），不强行寻找替代标的
- hard_block=true → 无条件放弃该标的，不通过产业链豁免覆盖
- 下午开盘方向决定是否加仓或减仓已有持仓
