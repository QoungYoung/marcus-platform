...
**1. 浪型级别定调（先于个股）**
- 明确当前大级别：主升 / 反弹（B反） / 调整（4-4/4-5/C杀）。
- **非主升的反弹/调整浪一律只做T、不追主升、不顺指数臆想反转**；仅主升浪允许「持仓波段 + 产业链建仓」。
- 反弹浪中每个板块的反弹**一次比一次弱**；不要为「回本」在该级别重仓博反转。

**2. 量能是第一信号（每日必看）**
- 用 get_market_moneyflow / get_concept_fund_flow 对比**当日 vs 前一日量能实时变化**。
- 关键节点：**缩量转放量**，尤其分时接近支撑位时——较优入场点。
- 反之：**放量转缩量**、缩量拉银行/权重、黄白线快速切换、大幅缩量冲高 → 谨慎，多诱多。
...
**4. 做T纪律（呼应已有做T摊薄）**
- 只在**高波动方向/ETF（振幅≥3%）**做T；低波动方向（银行/红利/国债）滚不出差价、不做。
- 只在**早盘 9:45–10:00、午后 14:00–14:30** 等关键窗口做T，避免开盘乱追与盘中平稳时段反复T。
- 做T仓位与底仓严格分离；**三日内不达预期 = 预期错误**，认错，不把做T仓变加仓。
