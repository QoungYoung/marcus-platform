    ...
    <div className="tac-page" ref={pageRef}>
        ...
        <div className="tac-section">
          <div className="tac-cards">
            <div className="tac-card">
              <div className="tac-card-label">初始资金</div>
              <div className="tac-card-value">{fmtMoney(capital?.initial_capital)}</div>
            </div>
            <div className="tac-card">
              <div className="tac-card-label">可用资金</div>
              <div className="tac-card-value">{fmtMoney(capital?.available_cash)}</div>
            </div>
            <div className="tac-card">
              <div className="tac-card-label">冻结资金</div>
              <div className="tac-card-value">{fmtMoney(capital?.frozen_cash)}</div>
            </div>
            <div className="tac-card">
              <div className="tac-card-label">持仓数</div>
              <div className="tac-card-value">{Object.keys(ledger).length}</div>
            </div>
          </div>

          <div className="tac-subtitle">当日可卖额度账本（T+1）</div>
        ...
        <div className="tac-section">
          ...
          {(['candidate', 'live', 'watch'] as const).map((tier) => (
            <div key={tier}>
              ...
              <table className="tac-table">
                <thead><tr><th>代码</th><th>得分</th><th>价差空间</th><th>O-C</th><th>往返度</th><th>说明</th></tr></thead>
                <tbody>
                  {Object.values(pool?.pool?.[tier] || {}).map((e) => (
                    <tr key={e.symbol}>
                      ...
                      <td>{e.round_trip ?? '-'}</td>
                      <td className="tac-muted">{(e.reason || []).join('; ')}</td>
                    </tr>
                  ))}
                  {Object.keys(pool?.pool?.[tier] || {}).length === 0 && (
                    <tr><td colSpan={6} className="tac-empty">暂无标的</td></tr>
                  )}
                </tbody>
              </table>
            </div>
      ...
      {tab === 'audit' && (
        <div className="tac-section">
          ...
          <div className="tac-cards tac-cards-sm">
            <div className="tac-card">
              <div className="tac-card-label">当日回转额</div>
              <div className="tac-card-value">{fmtMoney(audit?.daily_state?.daily_turnover_amount)}</div>
            </div>
            <div className="tac-card">
              <div className="tac-card-label">已实现盈亏</div>
              <div className="tac-card-value">{fmtMoney(audit?.daily_state?.realized_pnl)}</div>
            </div>
            <div className="tac-card">
              <div className="tac-card-label">买卖计数</div>
              <div className="tac-card-value">{audit?.daily_state?.buy_count ?? 0} 买 / {audit?.daily_state?.sell_count ?? 0} 卖</div>
            </div>
            <div className="tac-card">
              <div className="tac-card-label">熔断</div>
              <div className="tac-card-value">
                {audit?.risk_state?.stop_all ? '🔴 STOP_ALL' : audit?.daily_state?.risk_breaker ? '🔴 日亏熔断' : '🟢 正常'}
              </div>
            </div>
          </div>

          <div className="tac-subtitle">事件审计链（条件单 → 状态流转）</div>
          <table className="tac-table">
            <thead><tr>
      ...
      {tab === 'build' && (
        <div className="tac-section">
          ...
          <div className="tac-cards">
            <div className="tac-card">
              <div className="tac-card-label">t 净值</div>
              <div className="tac-card-value">{fmtMoney(buildOverview?.net_asset)}</div>
            </div>
            <div className="tac-card">
              <div className="tac-card-label">底仓市值</div>
              <div className="tac-card-value">{fmtMoney(buildOverview?.total_floor_value)}</div>
            </div>
            <div className="tac-card">
              <div className="tac-card-label">总底仓上限</div>
              <div className="tac-card-value">{fmtMoney(buildOverview?.total_floor_cap)}</div>
            </div>
            <div className="tac-card">
              <div className="tac-card-label">单笔/单标上限</div>
              <div className="tac-card-value tac-card-sm-value">
                {fmtMoney(buildOverview?.single_order_cap)} / {fmtMoney(buildOverview?.per_symbol_cap)}
              </div>
            </div>
          </div>
