...
INVENTORY = [
    ...
    # ── 建仓两道**真闸门**（2026-09-15 round 39 接入 / round 40 分位域改口径）────────────
    dict(id="gate.close_pos", cat="闸门", name="建仓门1：距前低**上三分位** ∧ 半强收盘(pos∈[POS_LO,POS_HI)) → 不买",
         loc="apps/main_line/wolf_entry_filters.py:close_pos_blocked",
         value="开（默认 1）；POS_LO=0.5 / POS_HI=0.8", source="env WOLF_CLOSE_POS_GATE 默认 + 代码默认",
         note="证据 §50 交叉格：n=369 −2.270%（块状 t −3.14），H1/H2 两段一致；同档其它腿不显著（弱证据，需生产观察）"),
    dict(id="gate.defensive", cat="闸门", name="建仓门2：距前低**下三分位** ∧ 近 N 日弱于大盘 → 不买（他的「抗跌」）",
         loc="apps/main_line/wolf_entry_filters.py:defensive_blocked",
         value="开（默认 1）", source="env WOLF_DEFENSIVE_GATE 默认",
         note="原话 2021-01-27「防御型就是大盘跌的时候他少跌一点」；「相对超额≥0」直接来自语义"),
    dict(id="gate.def_rel_win", cat="闸门", name="抗跌判据窗口（交易日）",
         loc="apps/main_line/wolf_entry_filters.py:def_rel_win", value="20",
         source="env WOLF_DEF_REL_WIN 默认", note="⛔自设：他没给窗口（§39 已标注）"),
    dict(id="gate.entry_qdomain", cat="闸门", name="两道建仓门的分位域口径（cand=当日候选域 / legs=当日终选腿）",
         loc="apps/main_line/wolf_entry_filters.py:qdomain_mode / filter_legs", value="cand（默认）；DOMAIN_MIN=6",
         source="env WOLF_ENTRY_QDOMAIN 默认",
         note="round 40 用户指令「分位改在更宽的候选域上算」；legs=旧口径（域内样本<6 → 门实际不生效）"),
    dict(id="gate.entry_min_closes", cat="闸门", name="算闸门特征（距前低/当日区间位置/20日涨幅）所需最少 K 线",
         loc="apps/main_line/wolf_entry_filters.py:MA_LINE_MIN_CLOSES", value="144",
         source="代码默认", note="与 144 线口径共用同一常量"),
    # 排序（v3，未上线）
...
BUY_MODULES = [
    ...
    "apps/main_line/wolf_entry_filters.py",
