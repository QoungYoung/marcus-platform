...
def main() -> int:
    ...
    # 2026-09-15（用户指示）：把 **PG 日线底座同步** 合并进这条 16:30 收盘后刷新链
    #   —— 此前 `mkt_bars_daily` 没有任何定时任务维护（tasks.yaml 里 mkt_bars/backfill 各 0 次），
    #   实测停在 20260911（缺 09-14），而中继取数完全可用（5550 行/2.8 秒）。
    try:
        from app.services import mkt_bars as MB
        print("[refresh_index_daily] PG 日线底座自愈:", MB.ensure_fresh(quiet=False))
    except Exception as _e:
        print("[refresh_index_daily] mkt_bars 同步失败(不影响指数刷新): %s" % str(_e)[:120])
    return 0
