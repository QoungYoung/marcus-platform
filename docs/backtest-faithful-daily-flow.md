...
### 9.12 第 6 轮（续）：**LLM 漂移检查抓到我自己的 PIT 修复** + 沙箱必须隔离

**① 漂移检查抓到了"我改了 as-of 口径"**：全窗口跑批重播 09-11/09-14 的 wave agent 时报

```
bt_llm_replay.LLMReplayError: prompt 与录制时不一致：agent=wave as_of=20260910
（录制 4ed1e6a8 / 现在 60bbe7eb） —— 输入数据/PIT 口径已漂移，回放不可比
```

逐行 diff 提示词后定位到**唯一**差异行 = `主线判定(main_line_state)`：

| | `main_line` | `date` | `main_line_source` |
|---|---|---|---|
| 录制时（早期轮次） | **农业** | 2026-09-13 | mainline_select_20260910 |
| 现在（修好的 as-of 选择） | **稳增长/基建** | 2026-09-11 | mainline_gate_20260910 |
| **生产归档真值** `_archive/20260911/main_line_state.json` | **稳增长/基建** | 2026-09-11 | mainline_gate_20260910（`updated_at 2026-09-11 08:00:46`） |

→ **录制的那版是"偷看了未来"的（09-13 的状态），现在的口径与生产一致**。也就是说：这条报错不是回归，
而是**漂移检查正确地抓住了我后来做的 PIT 修复**。处理方式：**换一个全新缓存目录 `_bt_llm_v2` 重录**
（`BT_LLM_MODE=record`），**绝不覆盖旧录制**（旧录制是当时口径的证据）。

**② 沙箱必须隔离（本轮最贵的教训）**：全窗口跑批（`--root` 未隔离，直接写 `_bt_full/<T>`）
把 09-11/09-14 的 `wave_state.json` **冲成空文件（2 字节 `{}`）** ——根因是上面那个 LLMReplayError 被
`bt_seed_day.py` 的 `try/except` 吞掉后**继续往下跑**（"wave 没跑成"却照样建沙箱、照样出腿=0），
于是 09-14 的 09:20 路径从"4 条腿全中"变成"0 条腿"，**而且是静默的**。
处置：
  - `bt_seed_day.py` / `bt_days.py` 增加 `--root`（沙箱根目录）与 `--sandbox` 透传 → 每轮跑批用独立根目录
    （本轮：`/app/data/_bt_sep`），不再与已验证的 `_bt_full` 混写；
  - LLM 缓存同样隔离（`BT_LLM_CACHE=/app/data/_bt_llm_v2`）；
  - **待办（下一轮）**：wave 再生 `rc!=0` 时 `bt_seed_day` 必须**显式失败**（写 `fatal` 字段 + 非 0 退出），
    `bt_days` 遇 `fatal` 标记该日"阻断"而不是记 0 条腿——否则"0 条腿"会被当成"确实没腿"，静默污染对账。

**③ 回放窗口的数据边界**：本地日线缓存 `_bt_full/bars.sqlite` 与生产 `mkt_bars_daily` 都**只到 2026-09-14**
（09-15 的日线尚未入库），所以本轮可复算窗口 = **2026-09-01 → 09-14**（09-15 待日线入库后补跑）。

**④ 新增对账工具** `jobs/bt_reconcile.py`：一次输出 4 节——`legs`（A 层调度通道 / B 层全部生产腿 / 回放三方对照）、
`triggers`（对回放腿跑 `bt_tape`，可选用**生产条件**或**回放条件**两种口径）、`fills`（回放撮合 vs `paper_trades`）、
`equity`（按成交 + 日线收盘 mark-to-market 的逐日净值）。A 层达标判定用 `recall_A`，B 层只作信息。

### 9.13 第 6 轮（续）：把"不可复现"从猜测变成**逐日账**（09-08 / 09-09 的根因是**输入版本选错**）

用沙箱自带的 `_seed.json` 清单（记录了每个输入文件的来源）逐日查"当日 `main_line_state.json` 到底取了哪份"：

| 决策日 | 沙箱取到的 | 结果 |
|---|---|---|
| 09-08 | `live_file_STALE?`（**当期活文件**；无归档/无专用日期文件/无 DB artifact） | 08:18 计划 0 条 ≠ 生产 13 条 |
| 09-09 | `live_file_STALE?` | 08:18 计划 5 条 ≠ 生产 10 条 |
| 09-10 | `daily_artifacts`（`orig=/app/data/main_line_state_20260909.json`） | 3 条 = 生产 3 条 ✅ |
| 09-11 | 归档/DB | 9 条 = 生产 9 条 ✅ |
| 09-14 | DB（cut=09-11 那份） | 4 条 = 生产 4 条 ✅ |

**关键发现**：`/app/data/main_line_state_20260909.json` **是存在的**（09-09 早晨那份），但播种脚本的选择链
（`_archive/<T>/main_line_state.json` → `_archive/<cut>/…` → DB artifact）**没有把"带日期的变体文件"列进去**，
于是 09-08/09-09 掉到了"活文件"兜底 —— 这就是 §9.8 里那句"有界缺口"的**真正根因**：不是数据丢了，
而是**选择链漏了一条**（09-08 是真丢：无归档、无 `main_line_state_20260908.json`、无 DB artifact；
09-09 没丢、只是没被选中）。

⇒ 可达上限（A 层 48 条腿）：
  - **可复现 35 条**（09-09 的 4 条 arm + 其 10 条 tranche〔修选择链后〕+ 09-10 8 + 09-11 9 + 09-14 4）；
  - **真丢 13 条**（09-08 的 tranche，那天的 `main_line_state` 三处都没有）。
  - 即"逐日全覆盖 ≥90%"在这份数据上**天花板约 73%**；若把口径限定在"输入可得的交易日"（09-09→09-14），
    可以做到 100%。**这两个数必须同时报**，否则"≥90% 达标"是拿分母做出来的。

**下一轮待做（已定位，改动是隔离的）**：
  ① `bt_seed_day.py` 的 `main_line_state` 选择链补上"带日期变体"（`_archive/<d>/main_line_state_<d>.json`、
     `<DATA_DIR>/main_line_state_<d>.json` 及横杠变体），顺序与"`updated_at ≤ T 日早晨`"校验一起写死；
...
### 9.21 第 7 轮（续）：版本自证工具 + "确认域"已能逐概念对齐（差在 **state.candidates**）

新增 `jobs/bt_pick_code_rev.py`：用**生产自己的日志**校验/挑选当天的代码版本 —— 两个条件缺一不可：
① `fusion_mainline.THEME_CONCEPTS` ⊇ 生产当天日志里的 `TOP确认主题`（`stock_confirm_refresh`）；
② 该版本树**必须含布腿链脚本**（`rotation_switch_arm.py` / `switch_builder.py` / `fusion_mainline.py` /
`stock_confirm_judge.py`）—— 只满足①的老树（如 `rev_b9266461`）是布腿链上线**之前**的版本，用它等于倒退。

实测（`--all` 扫全部 68 棵树）：

| 日 | `rev_map` 映射版本 | 主题数 | 判定 | 建议版本 |
|---|---|---|---|---|
| 09-09 | rev_a0b5912f | 9 | ❌ 缺 `传媒/游戏`、`农业` | **rev_6bc08f06**（14 主题+脚本齐全） |
| 09-10 | rev_5a6327… | 9 | ❌ 缺 `农业` | **rev_6bc08f06** |
| 09-11 | rev_1ce08503 | 14 | ✅ 满足（**该日回放本来就 9/9** → 自证工具与实测一致 ✔） | rev_1ce08503 |

换用 `rev_6bc08f06` 重跑 09-10 后，**确认域的映射对上了**：
沙箱 `{"农业种植": {"theme": "农业", ...}}`、且 `农业 > 乡村振兴 n=10 确认 1` 与生产日志
**逐字一致** ✔✔（引擎忠实）。但腿仍是 0，原因是**更上一层**：

| | 生产的 `TOP确认主题` | 沙箱实际处理 |
|---|---|---|
| 09-10 | `['农业', '金融', '稳增长/基建']`（= 当天 `main_line_state.candidates`） | 只有 `农业`（7 个概念） |

⇒ 差的是 **`main_line_state.json` 的版本**：09-10 早晨那份（`candidates` 三个主题）在归档/日期文件/DB
三处都没有，我的选择链退到了 09-09 那份（`candidates=['农业']`）→ 确认域只覆盖 1 个主题 → 无候选 → 0 条腿。
**下一轮固定动作**：把生产日志里能读到的当日 `TOP确认主题` / `GATE_CONFIRMED_TODAY` / `MAINLINE_POOL_TODAY`
**注入沙箱 state**（日志即生产产物，属可查证的输入），再跑 09-09/09-10/09-14。

**本轮"忠实"清单（已验证）**：09-11 腿级 9/9（两次）；确认引擎逐概念一致（乡村振兴 1=1）；
写入穿透 0（指纹自检）；09-01→09-07 回放 0 = 真值；对账/分叉/版本自证三件工具就绪。
