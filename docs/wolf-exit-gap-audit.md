...
|---|---|---|---|---|
| **R8** | **止损口径**（阶段 3，最大缺口） | 2026-08-27「**只看指数大级别**如果不走大5浪而转为下跌1浪就止损」（= 个股不止损） | **六层**（真正是止损的：① 13 日逻辑时间 ② 个股早停 −3% ③ 指数级 ④ 破位；⑤ 上影=停止做T、⑥ 中轨=止盈） | ✅ **对照已完成（2026-09-14，`docs/stop-layer-audit.md`，`jobs/eval_stop_layers.py`）**：n=428（+33 生产腿探索，20 日窗口）——个股级止损是**负贡献**（均值 +2.40→+0.79、胜率 43.9%→39.3%、止损正确率仅 46%、反事实 −1.45% 优于实际 −3.93%、离场后 10 日平均反弹 +6.72% > 续跌 −5.20%）；**只留指数级（MA60 代理）更好且尾部更小**（+1.29 / 最差5笔 −16.8% vs 个股级 −23.2%）；不止损尾部最差（−26.2%）。→ **证据支持"个股级降级为提示"**，但属行为改变，**等用户拍板**（选项见报告 §6：①降级 ②减半仓 ③不改） |
