...
|---|---|---|---|---|
| **R8** | **止损口径**（阶段 3，最大缺口） | 2026-08-27「**只看指数大级别**如果不走大5浪而转为下跌1浪就止损」 | **六层止损**：个股 −3% / 13 日 / 破位 / 上影 + BOLL 中轨 + **指数大级别**（`_check_index_level_stop`）——即"个股级止损"仍在跑 | 需先做对照：现状 vs「只指数止损」，指标 = 最大回撤 / 亏损尾部 / **被止损后 N 日反事实收益**（判断是否"割在最低点"）。若证据支持他 → 个股止损**降级为提示**（属行为改变，须用户确认） |
