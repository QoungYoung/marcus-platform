...
## 6 仍未与狼大一致的四条（2026-09-14 快照，供下一轮接手）

> 兑现侧（止盈/做T/避险/删票/分型）已按语料逐条落地；**下面四条是明确还没对齐的**，按优先级排序。

| # | 未对齐项 | 他的口径 | 我们的现状 | 为什么还没做 / 怎么做 |
|---|---|---|---|---|
| **R8** | **止损口径**（阶段 3，最大缺口） | 2026-08-27「**只看指数大级别**…转下跌1浪就止损」；2026-03-06「成趋势后→趋势线」+2021-01-28「13/34强弱、**60日底线**、没站稳34带量下穿就砍」 | 六层（13日逻辑 / 个股早停 / 指数级 / 破位） | ✅ **对照已完成 + 层②已落地（2026-09-14）**：① 对照见 `docs/stop-layer-audit.md`（n=428：①+0.79、②+2.01、③+1.29、①②③+0.98/尾部最好、不止损+2.40/尾部最差）；② 新增 `backend/app/services/wolf_trend_stop.py`（MA60 底线 / MA34 带量下穿 / 黑K破5日线减仓 / 13-34 强弱分类）+ `t_monitor._check_stop_loss` 阶段切换，开关 `WOLF_TREND_STOP`、`WOLF_TREND_KEEP_COND_STOP=0`（默认不保留 AI 价格线）、`WOLF_TREND_FRESH_ONLY=1`（上线护栏：久已破位只提示）；单测 10 项；已部署并在生产 4 个持仓上实测。**待用户决定**：3 个"久已破位"持仓要不要一并砍（`WOLF_TREND_FRESH_ONLY=0`） |
| **R9** | 不突破 + **卖单加大** + 无带动 | 语料里的出货确认判据（分时/盘口） | 未做 | **数据可得但无历史**：腾讯 qt 的 88 字段里有五档（卖一~卖五 = [19]~[28]），但 DB 无任何盘口快照表 → 只能做**实时判据、无法回测**。建议先"只存快照 + 只记录"，积累 4–8 周再决定是否上线（守住"无证据不上线"） |
| **ETF 档位** | ETF 与个股兑现幅度不同 | 2025-04-03「**T+0 2 个点我就够了**」；09-02 记录"ETF 能 T 出 2 个点即合格" | `ROUNDTRIP_SELL_UP` 全局 0.03（个股 3-5 个点下沿），**未按品种分档** | 最易补：按 symbol 是否 ETF 给 2%/3% 两档（`WOLF_ROUNDTRIP_SELL_UP_ETF=0.02`）。注意 ETF 的 T+0 语义与个股 T+1 不同，需一并把"当日可卖"判断对齐 |
| **R7/R10** | 尾段状态机 / 诱空最多三次 | 2026-09-04「一旦我认为行情结束」；2026-09-07 诱空 | 未做 | 判为**不可算化**（尾段只能用事后信息识别 = 前视；诱空需更细的盘口语义）→ 维持 ⛔，除非后续有可识别条件 |

另：**有意偏离（非缺口，已写明理由与开关）**——① 破线"确认"用 0.5%/2 轮（他只说"突发"，无数值，A3 阈值未定）；② 到点决断容忍浮亏 0.5%（"亏个手续费"无数值）；
③ 破线保护**全时段**生效（他原话是"在这个半小时内"，我们偏保守）；④ G5 的"拉升后都走"不做独立清仓腿（交给 G1 顶部判据 + C3/G2 兑现腿，避免第二套并行口径）；
⑤ R3「做 T 前置」（无低吸仓不做 T）**主动放开**（用户 2026-09-14 拍板开 C3：存量老仓也要有幅度止盈）；⑥ 删票 TTL=13 交易日为代理值（代码已注明）。
