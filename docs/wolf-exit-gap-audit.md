...
| **C2b** | **回补腿**（周一拿回 / 避险结束补回） | 「这样周一再拿回来」2026-08-21；「避险逻辑结束后 是不是应该补回来」2025-09-24 | ⏳ 待设计（用户要求单独设计） | 需新 trigger_kind + 冷却/上限；只减不加会系统性漂成低仓 |
...
| — | 纪律卖腿**账户口径**：board_half / profit_take / 中轨 / 避险 从"只读 t"改为 **stock + t** | t 是测试账户（用户 2026-09-13 明确） | ✅ 已落地 | `t_monitor._discipline_positions()`；`_insert_wolf_trigger(account_id=...)` |
...
- 未做：回补腿（C2b）——按用户要求单独设计；C1 的中轨腿目前因"顶部阶段"不满足而休眠（G1 待做）。
