...
| ① 后半句：逻辑与时间离场 | 2026-03-05 同句「**13日内需要碰新高或者新高。否则这个票呆的意义就不大，证明自己的买入逻辑和时间有问题**」 | `wolf_early_stop.logic_time_stop` + `t_monitor._check_logic_time_stop`：建仓后 13 交易日窗口走完仍**从未碰前高** → 离场（碰即算、盘中最高计入、窗口未走完不动作、数据不足不动作）。`WOLF_LOGIC_TIME_STOP=0` 关 |
  | ② 成趋势后→趋势线法 | 2026-03-06「成为趋势后…这个就没意义了…用**趋势线**的方法…不是一个策略用到底的」 | **语料无参数**（唯一"破5日/破趋势线"是 2022-04-26 一位用户自述，狼大未背书）→ **暂用 `stop_loss_price`**（用户 2026-09-10 决策：回测后再定口径）。⚠️ 该口径对 253/254 持仓**非无条件可用**（switch 腿布腿不带止损价，须触发过一次重建后才有）—— 见 §12.11(a) |
...
| **按市况分档给仓位（P3 三仓档位的骨架）** | `position_tier.py`：底仓/T仓/现金 × 浪型 operation → intent 是否允许 + cap_pct + cash_floor_pct（`P3_TIER_MODE=1` 硬拦） | ✅ **骨架有原话支撑（2026-09-11 复核更正）**：狼大 2026-01-17「**主升趋势就75%以上**…**调整就50%** **有风险就30%** **下跌趋势就不做**」；2022-05-23「等**一浪转二浪再打成70—80%仓位**」= **按浪型调仓位**；2025-02-11「固定仓位50%，剩下的做T等突破或回踩打满」；2025-04-17「60%固定仓位 其中20%地产 40%机器人…剩下的做当天的T」；2022-05-23「**必须保留一定现金**进行操作成本」。⚠️ **自造的只是命名与阈值**：`探仓`/`三仓`/`三档` 在语料 **0 命中**（`试仓` 3 条均为描述盘口/他人），cap 10/10/8/5/3% 与 cash_floor 25/30/35/45/50% **无语料** → 属"骨架有据、参数无出处"，与层①波段窗口/ U9 分位阈值同类（**该修参数与命名，不是删机制**） |
| **仓位下限与目标仓位分档** | ✅ **已落（2026-09-11）**：`wolf_discipline.position_cap.tier_targets` = build 75 / t_only 50 / side 50 / defense 30 / exit 50（按 operation 映射，**映射属外推**）；`tier_floor.build=55`；**硬拦侧同源** —— P3 现金底线 = 100 − 分档目标（`position_tier._tier_cash_floor`，单一事实来源） | 狼大 2026-01-17「**主升趋势就75%以上** 然后盘中满仓滚动啊 **调整就50%** **有风险就30%** **下跌趋势就不做**」（分档）+ 2025-08-11「这个位置 **仓位低于55%**…不太合适」（下限，**只挂 build 档**：原话带"这个位置"限定且他当时明确看多；挂到调整档会与 50% 目标数学互斥）。⚠️ **原 `total_max_pct=70` 的依据被复核推翻**：2026-03-06「现在就是70%仓位」是**状态描述、不是上限规则**，且与"主升75%以上…盘中满仓滚动"冲突 → 置 0，由分档取代。开关 `tier_enabled=false` / `P3_USE_TIER_TARGETS=0` |
