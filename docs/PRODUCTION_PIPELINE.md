...
| A4 **黄线上/白线上**判日内强弱 | 黄>白 → 主线强、**做T成功率高**；白>黄 → **减少做T** | 只有**个股级** `quote.vwap_break`（现价<个股均价）；**没有指数级"均价 vs 指数"的日内强弱字段** | ❌ 缺口（**重要**） |
| A5 **时间窗 9:45–10:00 / 14:00–14:30** | **只做这两个时段**（消息刺激股例外） | 只有**禁止**区间（14:45 后禁新开；13:00–14:30 禁止损）。`t_conditions.start_time/end_time` **两列存在但布腿时根本不写** | ❌ 缺口（需写代码，改动面小） |
...
| 优先级 | 缺口 | 为什么优先 | 可落地性 |
|---|---|---|---|
| **P1** | **A4 黄白线（指数级日内强弱）** | 它同时是 A4/A9 的前提，且他 2025-04-15 把它当作"做T成功率高不高"的总开关（2026-01-20/01-23 也在用） | **高** —— 指数 m5 里已有 amount/vol，**指数均价 = 累计额/累计量**可直接算 |
| **P1** | **A5 正向时间窗 9:45–10:00 / 14:00–14:30** | 他成文条件 2；我们只有禁止区间 | **高** —— `start_time/end_time` 两列已存在，布腿时写入 + 监控侧强制即可 |
