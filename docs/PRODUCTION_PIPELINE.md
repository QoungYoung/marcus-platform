...
11. **止损六层落地的口径与已知未定**（2026-09-10 立，2026-09-11 补 (c)(d)）：
    ...
    (d) **层①原话的**后半句**尚未落地（2026-09-11 新发现，待用户决策）**:
        同一条原话还有「**13日内需要碰新高或者新高。否则这个票呆的意义就不大，证明自己的买入逻辑和时间有问题**」
        —— 这是**时间/逻辑维度的离场**（不是价格止损），目前**无任何实现**。
        另：`t_conditions.time_stop_open` / `time_stop_close` 两列**被写入但从不被读**（`t_pool.py:290` 写 `"14:45"`，
        语义上其实是**日内时段**而非日期），属"机制静默失效"第 6 例。若要落地该半句，应同时纠正这两列的语义。
