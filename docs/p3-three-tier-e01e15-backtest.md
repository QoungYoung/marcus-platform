...
| block → 得到 P3 通道 | — | **+5（E02/E05/E08/E12/E13）** |
| partial → 得到 P3 通道 | — | **+5（E09/E10/E11/E14/E15）** |
| 仍无通道（设计保留） | — | 仅 E06（exit 无底仓抢反弹） |
...
| E02 | 只买一点 CPO(小仓试盘) | t_only | 无底仓 | ❌ | ✅probe≤3% | **2026-09-03 用户裁决**：t_only 增加 probe 试盘档（≤3%独立小仓，不追仓） |
| E03 | 继续买科技(回补) | build | T宇宙 | ✅放行 | ✅refill≤8% | 不变 |
| E04 | 半导体设备 ETF 换入 | build | 有底仓 | ✅放行 | ✅add≤10% | 不变 |
| E05 | 加深两只液冷 | t_only | 有底仓 | ❌不会新建 | ✅add≤5% | **新增：已有底仓加厚档** |
| E06 | 4000下买回AI硬 | exit | 无底仓 | ❌ | ❌ | **设计保留（用户裁决）**：exit+无底仓=左侧抢反弹，仍不放行 |
...
1. **旧系统缺的三个结构性通道现在有档位**：defense/t_only 期已有底仓加厚（E05/E10/E13/E14）、T资格回补（E12/E15）、t_only 小仓试盘 probe≤3%（E02）；E01-E15 中 block 的 6 个有 5 个获得通道、partial 的 5 个全部获得至少 T/回补通道。
2. **仅 E06 仍不放行（用户裁决）**：exit+无底仓回补=左侧抢反弹，与 wave gate 风控冲突，保持设计分歧。
